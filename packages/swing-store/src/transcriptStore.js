...
export function makeTranscriptStore(
  ...
  let enableTranscriptRolloverCompression = true;
  ...
  const sqlAddCompressedSpan = db.prepare(`
    INSERT INTO transcriptCompressedSpans (vatID, startPos, endPos, items, incarnation)
    VALUES (?, ?, ?, ?, ?)
  `);
  ...
  function compressSpan(vatID, startPos, endPos, incarnation) {
    ...
    for (const entry of readSpan(vatID, startPos)) {
      spanItems.push(entry);
    }
    spanItems.push(''); // in order to get the trailing newline in there
  ...
  function doSpanRollover(vatID, isNewIncarnation) {
    ...
    noteExport(spanMetadataKey(newRec), JSON.stringify(newRec));
    if (keepTranscripts) {
      if (enableTranscriptRolloverCompression) {
  ...
  function countSpanItems(vatID, rangeStartPos, rangeEndPos) {
    ...
    for (const { startPos, endPos } of sqlGetSpansInRange.iterate(
      ...
      const compressedSpanReader = readCompressedSpan(vatID, startPos);
      if (compressedSpanReader) {
        for (const _ of compressedSpanReader()) {
          count += 1;
        }
      } else {
  ...
  async function populateTranscriptSpan(name, makeChunkIterator, options) {
    ...
    const doCompression =
      enableTranscriptRolloverCompression && !metadata.isCurrent;
  ...
  /**
   * Fiddle with the flag enabling span compression.
   *
   * Normally compression is turned on, but some tests need to turn it off (at
   * least temporarily), and it's possible that at some point some backwards
   * compatibility issues may demand we have the capacity to modulate it.
   *
   * @param {boolean} mode
   */

  function setEnableTranscriptRolloverCompression(mode) {
    enableTranscriptRolloverCompression = mode;
  }
